...
//!
//! Two lines carry the review, and they are the last two: what reverts
//! without the proof noticing, and whether any of what does not revert lives
//! inside the tests. Everything above them is context.
...
fn status(verdict: &FileVerdict, glyphs: &Glyphs) -> String {
    if verdict.tampered {
        ...
        format!("unproven {} revert-safe", glyphs.dash)
    } else {
        "proven".to_string()
    }
}
...
pub fn render_map(
    ...
    match map.outcome {
        MapOutcome::Mapped => {
            out.push_str(&format!(
                "  {} claim discharged        proof: {proof_source}{origin}\n",
                glyphs.tick
            ));
            out.push_str(&format!(
                "  {} proof coverage  {:<6}  {} of {} hunks load-bearing\n",
                glyphs.gauge,
                map.coverage.to_string(),
                map.load_bearing.len(),
                map.load_bearing.len() + map.unproven.len()
            ));
        }
    ...
    // Load-bearing first, then unproven, then the flagged ones last so the
    // reader's eye ends on the thing that matters most.
    let mut files: Vec<&FileVerdict> = map.files.iter().collect();
    files.sort_by_key(|f| (f.tampered, f.is_entirely_unproven(), f.path.clone()));
...
mod tests {
    ...
    fn a_mapped_run_reads_like_the_documented_output() {
        ...
        // 8 of 19 hunks load-bearing, 80 of 190 changed lines.
        let text = render_map(&map, "pytest -q", true, &Glyphs::UNICODE);
        assert!(text.contains("claim discharged"));
        assert!(text.contains("proof: pytest -q (default)"));
        assert!(text.contains("proof coverage  42%"));
        assert!(text.contains("8 of 19 hunks load-bearing"));
        assert!(text.contains("unproven — revert-safe"));
